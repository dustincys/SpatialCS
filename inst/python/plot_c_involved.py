...
class C_I_Plot(DrawOutline):
    ...
    def _dim_plot_ha_cluster(
        ...
        if highlight_PAs_fill is not None:
            ...
            mapping_array = pd.Series(
                [i + 1 for i in range(len(highlight_PAs_fill))],
                index=highlight_PAs_fill,
            )
            ...
            self._cluster_table["pathology_annotation_temp"][
                ~np.isin(
                    self._cluster_table["pathology_annotation"], highlight_PAs_fill
                )
            ] = "NOTSHOW"
    ...
    def _dim_plot_cluster_ha(
        ...
        if highlight_PAs_fill is not None:
            ...
            # Here based on the cluster index remain, calculate the fraction ######
            c_involved = (
                self._cluster_table[
                    np.isin(
                        self._cluster_table["pathology_annotation"], highlight_PAs_fill
            ...
            c_involved["frac"] = c_involved["n"] / c_involved["n"].sum()
            c_involved = c_involved[
                (c_involved["n"] > min_n) | (c_involved["frac"] > min_frac)
            ]
            c_involved = c_involved["cluster"].unique()
            ...
            self._cluster_table["cluster_index_temp"][
                np.logical_and(
                    ~np.isin(self._cluster_table["cluster"], c_involved),
                    ~np.isin(
                        self._cluster_table["pathology_annotation"], highlight_PAs_fill
                    ),
                )
            ...
            cluster_index_remain = self._cluster_table["cluster_index"][
                self._cluster_table["cluster_index_temp"] != -1
            ]

            self._c_involved_frac = round(
                (self._cluster_table["cluster_index_temp"] != -1).sum()
                / (
                    np.isin(
                        self._cluster_table["pathology_annotation"], highlight_PAs_fill
                    )
    ...
    def combined_dim_plot(
        ...
        highlight_PAs_border=None,
        out_folder_path=".",
        out_file_name="spatial_hac_c_involved.pdf",
        min_frac=0.1,
        min_n=10,
        avg_rogue_pa=0.0,
        avg_rogue_cluster=0.0,
    ):
        ...
        if highlight_PAs_fill is not None:
            ...
            plt.title("avg_Rogue_pa = {}".format(str(avg_rogue_pa)))
        ...
        if highlight_PAs_fill is not None:
            ...
            plt.title(
                "c_involved_frac = {0}, avg_rogue_cluster={1}".format(
                    str(self._c_involved_frac),
                    str(avg_rogue_cluster),
                )
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="average rogue of ha fill",
    )
    parser.add_argument(
        "--clusterAvgRogue",
        dest="clusterAvgRogue",
        type=float,
        default=0.0,
        help="average rogue of ha cluster involved",
    )
    ...
    hac.combined_dim_plot(
        ...
        min_n=args.minN,
        avg_rogue_pa=args.haAvgRogue,
        avg_rogue_cluster=args.clusterAvgRogue,
    )
